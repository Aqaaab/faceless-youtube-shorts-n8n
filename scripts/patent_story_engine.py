...
def generate():
    ...
    excluded=set(); previous_error=''; attempts=0; last=None
    ...
    for attempt in range(1,max_attempts+1):
        ...
        try:
            ...
            result,current_provider,model=router.route(prompt,exclude=excluded)
            ...
            msg=str(e).lower(); is_schema=any(x in msg for x in ('scene count','word count','language contract','missing story beats','invalid long-form','invalid tags','visual/query length contract','required fields','invalid beat','schema_invalid','no json object','invalid json object'))
            if is_schema and current_provider:
            ...
            if current_provider and current_provider in excluded and len(excluded)>=len(router.providers)-1:
                excluded=set()
