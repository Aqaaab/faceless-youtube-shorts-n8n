...
def clean_cartesia(text: str) -> str:
...
def eleven(text, out):
    ...
    subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-i',tmp,'-ar','48000','-ac','2','-c:a','pcm_s16le',str(out)],check=True)
...
def cartesia(text, out):
    ...
    body={'model_id':model,'transcript':clean_cartesia(text),'voice':{'mode':'id','id':voice},'output_format':{'container':'wav','encoding':'pcm_s16le','sample_rate':44100},'language':'en','generation_config':{'speed':float(os.getenv('CARTESIA_SPEED','1.0')),'volume':1.0}}
...
def kokoro(text,out,argv):
    bin=os.getenv('KOKORO_FALLBACK_BIN') or (argv[0] if argv else '')
    if not bin: raise RuntimeError('Kokoro fallback binary missing')
    tmp=str(out)+'.kokoro.wav'
    subprocess.run([bin, *argv[1:-2], tmp], check=True) if False else None
    raise RuntimeError('Kokoro fallback is handled by the shell wrapper')

def main():
    if len(sys.argv)<3: raise SystemExit('usage: voice_router.py TEXT_FILE OUTPUT_WAV')
    text=Path(sys.argv[1]).read_text(encoding='utf-8').strip(); out=Path(sys.argv[2]); out.parent.mkdir(parents=True,exist_ok=True)
    errors=[]
    for name,fn in [('ElevenLabs',eleven),('Cartesia',cartesia)]:
        try:
            fn(text,out)
            if out.exists() and out.stat().st_size>1000:
                print(f'VOICE_PROVIDER={name}'); return 0
        except Exception as e:
            errors.append(f'{name}: {e}'); print(f'VOICE_PROVIDER={name} FAILED: {e}',file=sys.stderr)
