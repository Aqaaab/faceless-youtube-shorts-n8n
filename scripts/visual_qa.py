...
STRICT=float(os.environ.get('VISION_MIN_SCORE','0.88')); MIN_SEMANTIC=float(os.environ.get('VISION_MIN_SEMANTIC_SCORE','0.85')); MIN_DIVERSITY=.18
...
def vision(prompt,images):
 ...
 return evaluate(prompt,images,'final-qa')
def positive_translation(reason,ar):
 r=str(reason or '').lower()
 return bool(re.search(r'[\u0600-\u06ff]',ar)) and any(x in r for x in ('faithful','accurate','correct','fluent','faithfully'))
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('run_dir'); run=Path(ap.parse_args().run_dir); job=json.loads((run/'job.json').read_text(encoding='utf-8')); scenes=job.get('scenes') or []
 ...
 for i,s in enumerate(scenes,1):
  ...
  reports.append(r)
 measured=diversity([run/'scenes'/f'scene_{i}.mp4' for i in range(1,6)]) if all((run/'scenes'/f'scene_{i}.mp4').exists() for i in range(1,6)) else 0.0
 provider_errors=[]; model='none'
 if len(sheets)==5:
  prompt='''Strict visual publication gate. Judge actual footage against the exact narration. Generic object presence is insufficient when narration describes a mechanism. For waggle dance duration/direction/distance, footage should visibly show bee movement/dance or an equivalent clearly observable behavior; a static hive is insufficient. For guiding a colony through movement, feeding on flowers alone is insufficient. Reject semantic adjacency. Arabic must be faithful Modern Standard Arabic. Return ONLY JSON {"scenes":[{"scene":1,"visual_match":true,"visual_score":0.95,"semantic_score":0.92,"translation_ok":true,"reason":"...","translation_reason":"..."}],"diversity_score":0.30,"overall_pass":true}. Exactly five scenes.'''
  ...
  try: result=vision(prompt,sheets); model='vision-agent'
  ...
  if not result:
   ...
   model_div=float(result.get('diversity_score',0) or 0); final_div=min(measured,model_div) if model_div>0 else measured
   ...
   for i,r in enumerate(reports,1):
    x=by.get(i,{})
    vs=float(x.get('visual_score',0) or 0); ss=float(x.get('semantic_score',0) or 0); vm=bool(x.get('visual_match')); tr=bool(x.get('translation_ok')) and r['translation_ok']; reason=str(x.get('reason','')); treason=str(x.get('translation_reason',''))
    # Some models return translation_ok=false while explicitly saying the Arabic is faithful/accurate.
    if not tr and positive_translation(treason,r['text_ar']): tr=True
    ok=vm and vs>=STRICT and ss>=MIN_SEMANTIC and tr and final_div>=MIN_DIVERSITY
    r.update({'visual_match':vm,'visual_score':vs,'semantic_score':ss,'translation_ok':tr,'diversity_score':final_div,'reason':reason,'translation_reason':treason,'passed':ok})
 ...
 final={'passed':not failures,'model':model,'provider_errors':provider_errors,'diversity_score':measured,'failures':failures,'scenes':reports,'thresholds':{'visual_score':STRICT,'semantic_score':MIN_SEMANTIC,'diversity_score':MIN_DIVERSITY}}
