...
def main():
 ...
 if not MIN_SCENES<=len(scenes)<=MAX_SCENES:
  failures.append({'type':'scene_count','reason':f'expected {MIN_SCENES}-{MAX_SCENES} scenes, got {len(scenes)}'})
 ...
 for i,s in enumerate(scenes,1):
  ...
  reports.append(r)
 scene_paths=[run/'scenes'/f'scene_{i}.mp4' for i in range(1,len(scenes)+1)]
 measured=diversity(scene_paths) if scenes and all(p.exists() for p in scene_paths) else 0.0
 provider_errors=[]; model='none'
 if len(sheets)==len(scenes) and MIN_SCENES<=len(scenes)<=MAX_SCENES:
  prompt=f'''Strict visual publication gate. Judge actual footage against the exact narration, not merely the presence of a related object. Generic object presence is insufficient when narration describes a mechanism or action. Reject semantic adjacency. Arabic must be faithful Modern Standard Arabic. Return ONLY JSON with this exact structure: {{"scenes":[{{"scene":1,"visual_match":true,"visual_score":0.95,"semantic_score":0.92,"translation_ok":true,"reason":"...","translation_reason":"..."}}],"diversity_score":0.30,"overall_pass":true}}. Exactly {len(scenes)} scene assessments, one for every scene.'''
