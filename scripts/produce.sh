...
MIN_SCENES="${MIN_SCENES:-6}"; MAX_SCENES="${MAX_SCENES:-10}"
...
[[ "$SCENE_COUNT" -ge "$MIN_SCENES" && "$SCENE_COUNT" -le "$MAX_SCENES" ]] || { echo "ERROR: scene count $SCENE_COUNT outside $MIN_SCENES-$MAX_SCENES" >&2; exit 1; }
...
for ((i=1;i<=SCENE_COUNT;i++)); do
 ...
 bash "$GITHUB_WORKSPACE/scripts/voice_router.sh" "$text_file" "$audio"
 [[ -s "$audio" ]] || { echo "ERROR: no voice audio for scene $i" >&2; exit 1; }
 dur="$(duration "$audio")"
 python "$GITHUB_WORKSPACE/scripts/visual_candidate_select.py" "$query" "$subject" "$en" "$source"
 [[ -s "$source" ]] || { echo "ERROR: no visual provider succeeded for scene $i" >&2; exit 1; }
...
if [[ "$MUSIC_ENABLED" == "true" ]]; then
 MUSIC_PATH="${MUSIC_FILE:-}"; for candidate in "$MUSIC_PATH" "$RUN_DIR/music/music.mp3" "$RUN_DIR/music/background.mp3" "${GITHUB_WORKSPACE:-}/assets/music.mp3"; do if [[ -n "$candidate" && -s "$candidate" ]];then MUSIC_PATH="$candidate";break;fi;done
 if [[ -n "${MUSIC_PATH:-}" && -s "$MUSIC_PATH" ]];then vd="$(duration "$AUDIO_FINAL")"; ffmpeg -hide_banner -loglevel error -y -stream_loop -1 -i "$MUSIC_PATH" -i "$AUDIO_FINAL" -filter_complex "[0:a]volume=${MUSIC_VOLUME},atrim=0:${vd},asetpts=N/SR/TB[m];[1:a][m]amix=inputs=2:duration=first:normalize=0:dropout_transition=2[a]" -map "[a]" -ar 48000 -ac 2 -c:a pcm_s16le "$RUN_DIR/audio/final_mix.wav"; AUDIO_FINAL="$RUN_DIR/audio/final_mix.wav";fi
fi
...
echo "Production complete: $RUN_DIR/video.mp4; scenes=$SCENE_COUNT"