...
wrap_ar(){ awk -v text="$1" -v max=42 'BEGIN{n=split(text,w,/ +/);line="";out="";for(i=1;i<=n;i++){if(line=="")line=w[i];else if(length(line)+1+length(w[i])<=max)line=line" "w[i];else{out=out (out==""?"":"\\N") line;line=w[i]}}if(line!="")out=out (out==""?"":"\\N") line;print out}'; }
render_scene(){
  local src="$1" dur="$2" out="$3" vf
  if [[ "$FORMAT" == "patent" || "$FORMAT" == "long_form" ]]; then
    if [[ "$ANIMATION_ENABLED" == "true" ]]; then
      vf="scale=2134:1200:force_original_aspect_ratio=increase,crop=2134:1200,zoompan=z='min(zoom+0.003333,1.10)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=1920x1080:fps=30,setsar=1,format=yuv420p"
    else
...
Style: AR,DejaVu Sans,54,&H0000FFFF,&H0000FFFF,&H00101010,&HE6000000,1,0,0,0,100,100,0,0,3,2,1,2,90,90,70,1
[Events]
Format: Layer,Start,End,Style,Name,MarginL,MarginR,MarginV,Effect,Text
EOF
: > "$RUN_DIR/video/scenes.txt"; : > "$RUN_DIR/audio/audio_concat.txt"; start=0
for ((i=1;i<=SCENE_COUNT;i++)); do
  idx=$((i-1)); en="$(jq -r ".scenes[$idx].text_en // empty" "$JOB_FILE")"; ar="$(jq -r ".scenes[$idx].text_ar // empty" "$JOB_FILE")"; query="$(jq -r ".scenes[$idx].pexels_query // empty" "$JOB_FILE")"; subject="$(jq -r ".scenes[$idx].visual_subject // empty" "$JOB_FILE")"
  ...
  bash "$GITHUB_WORKSPACE/scripts/voice_router.sh" "$text_file" "$audio"; [[ -s "$audio" ]] || { echo "ERROR: no voice audio for scene $i" >&2; exit 1; }
  dur="$(duration "$audio")"; awk -v d="$dur" 'BEGIN{exit !(d>0)}' || { echo "ERROR: invalid scene audio duration $dur" >&2; exit 1; }
  python "$GITHUB_WORKSPACE/scripts/visual_candidate_select.py" "$query" "$subject" "$en" "$source"; [[ -s "$source" ]] || { echo "ERROR: no visual provider succeeded for scene $i" >&2; exit 1; }
  render_scene "$source" "$dur" "$scene"
  printf "file '%s'\n" "$scene" >> "$RUN_DIR/video/scenes.txt"; printf "file '%s'\n" "$audio" >> "$RUN_DIR/audio/audio_concat.txt"
  end="$(awk -v a="$start" -v b="$dur" 'BEGIN{printf "%.3f",a+b}')"; wrapped="$(wrap_ar "$ar")"
  printf 'Dialogue: 0,%s,%s,AR,,0,0,0,,{\\fs54}%s\n' "$(ass_time "$start")" "$(ass_time "$end")" "$(ass_escape "$wrapped")" >> "$RUN_DIR/subtitles/subtitles.ass"; start="$end"
done
...
if [[ "$MUSIC_ENABLED" == "true" ]]; then
  ...
  vd="$(duration "$AUDIO_FINAL")"; ffmpeg -hide_banner -loglevel error -y -stream_loop -1 -i "$MUSIC_PATH" -i "$AUDIO_FINAL" -filter_complex "[0:a]volume=${MUSIC_VOLUME},atrim=0:${vd},asetpts=N/SR/TB[m];[1:a][m]amix=inputs=2:duration=first:normalize=0:dropout_transition=2[a]" -map "[a]" -ar 48000 -ac 2 -c:a pcm_s16le "$RUN_DIR/audio/final_mix.wav"; AUDIO_FINAL="$RUN_DIR/audio/final_mix.wav"; MUSIC_PRESENT=true
fi
[[ "$MUSIC_ENABLED" != "true" || "$MUSIC_PRESENT" == "true" ]] || { echo "ERROR: music is required but was not mixed" >&2; exit 1; }
if [[ "$MUSIC_PRESENT" == "true" ]]; then [[ -s "$RUN_DIR/audio/final_mix.wav" ]] || { echo "ERROR: final mix missing" >&2; exit 1; }; voice_hash="$(sha256sum "$RUN_DIR/audio/voice.wav" | awk '{print $1}')"; mix_hash="$(sha256sum "$RUN_DIR/audio/final_mix.wav" | awk '{print $1}')"; [[ "$voice_hash" != "$mix_hash" ]] || { echo "ERROR: music mix is byte-identical to voice-only audio" >&2; exit 1; }; fi
...
FINAL_SECONDS="$(duration "$RUN_DIR/video.mp4")"; awk -v d="$FINAL_SECONDS" -v lo="$MIN_VIDEO_SECONDS" -v hi="$MAX_VIDEO_SECONDS" 'BEGIN{exit !(d>=lo && d<=hi)}' || { echo "ERROR: final duration ${FINAL_SECONDS}s outside ${MIN_VIDEO_SECONDS}-${MAX_VIDEO_SECONDS}s for format=$FORMAT" >&2; exit 1; }
python - "$RUN_DIR" "$MUSIC_PRESENT" "$ANIMATION_ENABLED" "$FORMAT" "$FINAL_SECONDS" "$WIDTH" "$HEIGHT" <<'PY'
import json,sys,os
from pathlib import Path
run=Path(sys.argv[1]); music=sys.argv[2]=='true'; anim=sys.argv[3]=='true'; fmt=sys.argv[4]; duration=float(sys.argv[5]); width=int(sys.argv[6]); height=int(sys.argv[7])
...
echo "Production complete: $RUN_DIR/video.mp4; format=$(jq -r '.format' "$RUN_DIR/render_contract.json"); resolution=$(jq -r '.resolution.width' "$RUN_DIR/render_contract.json")x$(jq -r '.resolution.height' "$RUN_DIR/render_contract.json"); duration=$(jq -r '.duration_seconds' "$RUN_DIR/render_contract.json")s; provider=$(jq -r '.ai_provider.provider' "$RUN_DIR/render_contract.json")"