...
for ((i=1;i<=SCENE_COUNT;i++)); do
  idx=$((i-1))
  en="$(jq -r ".scenes[$idx].text_en // empty" "$JOB_FILE")"
  ar="$(jq -r ".scenes[$idx].text_ar // empty" "$JOB_FILE")"
  ...
  dur="$(duration "$audio")"
  awk -v d="$dur" 'BEGIN{exit !(d>0)}' || { echo "ERROR: invalid scene audio duration $dur" >&2; exit 1; }
