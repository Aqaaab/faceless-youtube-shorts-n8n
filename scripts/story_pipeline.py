...
def validate_story(story:dict)->None:
    ...
    if not isinstance(scenes,list) or len(scenes)!=CFG['production']['long_scene_count']: raise ValueError('story must contain exactly 25 scenes')
    for i,sc in enumerate(scenes,1):
        if not all(str(sc.get(k,'')).strip() for k in ('text_en','text_ar','visual_subject','pexels_query','beat')): raise ValueError(f'scene {i} missing required fields')
        n=words(sc['text_en'])
        if not 45<=n<=70: raise ValueError(f'scene {i} has invalid English word count: {n}')
        if re.search(r'[\u0600-\u06ff]',sc['text_en']): raise ValueError(f'scene {i} English contains Arabic')
        if not re.search(r'[\u0600-\u06ff]',sc['text_ar']): raise ValueError(f'scene {i} Arabic missing')
