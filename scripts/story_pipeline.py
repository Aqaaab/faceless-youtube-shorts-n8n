...
def generate() -> dict:
    ...
    # Deliberately no direct provider fallback: Odysseus owns routing, model
    # selection, retries and provider failover outside this repository.
    story = extract_json(call(message, model=model))
    validate_story(story)
    story["provider"] = "Odysseus"
    (run / "long_story.json").write_text(
        json.dumps(story, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    print(f"STORY_GENERATION=PASS provider=Odysseus scenes={len(story['scenes'])}")
    return story
