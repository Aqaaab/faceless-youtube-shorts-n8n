...
# Support both execution modes used by production and validation:
#   python scripts/odysseus_primary_story.py
#   python -c 'from scripts.odysseus_primary_story import ...'
# The story engine and router live beside this module in scripts/.
...
def generate():
        ...
        while not completed:
            ...
            if odysseus_active:
                try:
                    ...
                    slot_results.append({"slot_id": slot["slot_id"], "start_scene": slot["start_scene"], "end_scene": slot["end_scene"], "provider": "Odysseus", "model": MODEL, "attempt": attempt, "words": words, "status": "PASS", "route": "primary"})
                    prior_tail = scenes[-2:]
                    completed = True
                    print(f"LONG_STORY_SLOT_PASS slot={slot['slot_id']} provider=Odysseus route=primary attempt={attempt}")
                    continue
                except Exception as exc:
                    last = exc
                    slot_error = str(exc)
            ...
            try:
                ...
                all_scenes.extend(scenes)
                slot_results.append({"slot_id": slot["slot_id"], "start_scene": slot["start_scene"], "end_scene": slot["end_scene"], "provider": provider, "model": model, "attempt": attempt, "words": words, "status": "PASS", "route": "router_fallback"})
                prior_tail = scenes[-2:]
                completed = True
                print(f"LONG_STORY_SLOT_PASS slot={slot['slot_id']} provider={provider} route=router_fallback attempt={attempt}")
            except Exception as exc:
                last = exc
                slot_error = str(exc)
    ...
    merged = {"title": title, "description": description, "tags": tags, "topic": winner.get("topic"), "category": "Stories", "selected_hook": winner.get("selected_hook"), "scenes": all_scenes, "slot_results": slot_results, "router": "Odysseus Primary Intelligence Gateway", "router_task": "long_story_slots", "provider": "multi_provider_slots", "model": "odysseus_primary_with_router_fallback", "story_mode": "fixed_slots", "idea_council_winner": winner}
    validate_final(merged)
    OUT.write_text(json.dumps(merged, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"LONG_STORY_PASS mode=fixed_slots intelligence=odysseus_primary slots={len(SLOTS)} scenes={merged['scene_count']} words={merged['script_words']} hook_injected={bool(merged.get('selected_hook'))}")
