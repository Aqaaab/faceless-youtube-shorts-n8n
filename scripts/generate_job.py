...
Return ONLY one JSON object. No Markdown, commentary, or explanations.
...
Title: English only, <=90 characters, ending with #Shorts.
...
Scenes are authoritative. hook, script, subtitle_ar, query, topic, and category may be omitted because the program derives them deterministically.
"""

# OpenRouter structured output uses a deliberately small authoritative schema.
# Derived fields are filled by normalize(), avoiding failures such as "Missing field: hook".
OPENROUTER_SCHEMA = {
    "type": "object",
    "properties": {
        "title": {"type": "string"},
...
def request_openrouter() -> dict:
    ...
    body = {
        "model": model,
        "messages": [
            {"role": "system", "content": "Return exactly one valid JSON object matching the supplied schema. Do not omit scenes."},
...
def extract_openrouter(result: dict) -> dict:
    ...
    # Some providers expose the text through a content object; support that too.
...
def normalize(data: dict) -> dict:
    """Derive non-authoritative metadata from the five authoritative scenes."""
    ...
    data["topic"] = str(data.get("topic") or data["query"]).strip()
...
def try_provider(name, request_fn, extract_fn, attempts: int) -> bool:
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            print(f"{name} HTTP {exc.code}: {detail[:1200]}")
            if exc.code == 429:
                print(f"{name} quota/rate limit; switching to next provider")
                return False
            if exc.code in {400, 401, 403, 404}:
                print(f"{name} request rejected; switching to next provider")
                return False
...
def main() -> None:
    if try_provider("OpenRouter", request_openrouter, extract_openrouter, 2):
        return
