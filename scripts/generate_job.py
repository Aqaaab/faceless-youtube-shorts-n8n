...
def openrouter(k,prompt):
 x=post(OPENROUTER_URL,{'model':OPENROUTER_MODEL,'messages':[{'role':'system','content':'Return exactly one JSON object. No markdown.'},{'role':'user','content':prompt}],'temperature':.1,'max_tokens':5000},{'Authorization':f'Bearer {k}','Content-Type':'application/json','HTTP-Referer':'https://github.com/Aqaaab/faceless-youtube-shorts-n8n','X-Title':'Faceless YouTube Shorts'}); return extract(((x.get('choices') or [{}])[0].get('message') or {}).get('content',''))
def gemini(k,prompt):
...
def publication_metadata(d,sc):
 topic=str(d.get('topic') or d.get('query') or sc[0].get('visual_subject') or 'Science').strip(); topic=re.sub(r'\s+',' ',topic); clean=re.sub(r'[^A-Za-z0-9 ,:!?\-]','',topic).strip(' -,:') or 'Science'
