...
MODELS = []
for model in [configured, "gemini-2.5-flash", "gemini-2.0-flash"]:
...
def validate(data: dict) -> None:
    ...
    if not 85 <= scene_words <= 105:
        raise ValueError(f"Scene narration has {scene_words} words; expected 85-105")
    for i, scene in enumerate(scenes, 1):
        ...
        if not 10 <= words(scene["text_en"]) <= 25:
            raise ValueError(f"Scene {i} English text length is invalid")
        if re.search(r"[\u0600-\u06ff]", scene["text_en"]):
            raise ValueError(f"Scene {i} English text contains Arabic")
        if len(scene["pexels_query"].split()) > 3:
            raise ValueError(f"Scene {i} Pexels query is too long")
    if not str(data["title"]).endswith("#Shorts"):
...
def main() -> None:
    ...
    for model in MODELS:
        for attempt in range(1, 5):
            ...
            try:
                data = extract_json(request(model))
                validate(data)
    ...
    # Keep the renderer contract explicit and stable.
