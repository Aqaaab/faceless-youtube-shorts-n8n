#!/usr/bin/env python3
"""Router-backed long-story generator. Existing patent_story_engine.py remains untouched."""
...
def main():
    ...
    council=json.loads(COUNCIL.read_text(encoding='utf-8')); winner=council.get('winner')
    if not winner or winner.get('status') not in ('winner',None): raise SystemExit('INVALID_IDEA_COUNCIL_WINNER')
    context=json.dumps({k:winner.get(k) for k in ('topic','core_question','hook','novel_angle','source_pattern')},ensure_ascii=False)
    base=f'''Use this approved Idea Generation Council winner as the sole story concept. Do not copy its source pattern, title, wording, scenes or claims. Create an independent factual or clearly framed true-story story from the approved concept. Council winner: {context}\n\n{PROMPT}'''
    from ai_router import build_long_story_router
    router=build_long_story_router(); last=None
    for attempt,prompt in enumerate((base,base+'\n'+REPAIR),1):
        try:
            result,provider,model=router.route(prompt)
            d=validate(result); d.update({'provider':provider,'model':model,'router':'aqaaab-ai-router','generation_attempt':attempt,'idea_council_winner':winner})
            OUT.write_text(json.dumps(d,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
            print(f'LONG_STORY_ROUTER=PASS provider={provider} model={model or "default"} scenes={d["scene_count"]} words={d["script_words"]}')
            return
        except Exception as e:
            last=e; print(f'LONG_STORY_ROUTER_ATTEMPT={attempt} FAIL reason={e}')
    raise SystemExit(f'LONG_STORY_ROUTER_FAILED: {last}')
