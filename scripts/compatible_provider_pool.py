...
def generate(name:str,prompt:str)->dict[str,Any]:
 ...
 model=os.getenv(f"{name.upper()}_MODEL",cfg["model"]);body={"model":model,"messages":[{"role":"system","content":"Return exactly one valid JSON object. No markdown, no prose outside JSON."},{"role":"user","content":prompt}],"temperature":0.1,"max_tokens":12000}
