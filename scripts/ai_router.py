...
def _classify(exc):
    ...
    if any(x in m for x in ("401", "unauthorized", "invalid api key")):
        return "AUTH"
    if any(x in m for x in ("402", "payment_required", "payment required")):
        return "PAID_REQUIRED"
    if any(x in m for x in ("403", "accessdenied", "unpurchased", "allocationquota.freetieronly")):
        return "ACCESS_OR_QUOTA"
    if any(x in m for x in ("404", "model_not_found", "model not found")):
...
class AIRouter:
    def __init__(self, providers, task="long_story"):
        if bool(_load_config().get("free_only", True)) and any(not p.free_only for p in providers):
    ...
    def _entry(self, name):
        return self.state.setdefault("providers", {}).setdefault(name, {
            "status": "UNKNOWN", "failures": 0, "calls": 0,
            "estimated_tokens": 0, "cooldown_until": 0, "last_error": ""
        })
    ...
    def _eligible(self, p):
        e = self._entry(p.name)
        return time.time() >= float(e.get("cooldown_until", 0))

    def route(self, prompt, exclude: set[str] | None = None):
        exclude = exclude or set()
        required = estimate_tokens(prompt)
        ledger = []
        for p in self.providers:
            if p.name in exclude:
                ledger.append({"provider": p.name, "decision": "SKIP_EXCLUDED", "required_tokens": required})
                continue
            ...
            try:
                ...
                ledger.append({"provider": p.name, "decision": "FAIL", "classification": kind, "error": str(e)[:700], "model": p.model})
        self._write(ledger)
...
def _compat(provider, key, model, prompt, base_url=None):
    ...
    body = {"model": model, "messages": [{"role": "system", "content": "Return exactly one JSON object. No markdown."}, {"role": "user", "content": prompt}], "temperature": 0.1, "max_tokens": 12000, "response_format": {"type": "json_object"}}
    try:
        x = _http_post(url, body, {"Authorization": f"Bearer {key}", "Content-Type": "application/json"})
    except Exception as e:
        if "400" not in str(e).lower() and "response_format" not in str(e).lower():
            raise
...
def _blockrun(prompt):
    preferred = ["nvidia/nemotron-3-nano-omni-30b-a3b-reasoning", "nvidia/nemotron-nano-12b-v2-vl", "nvidia/nemotron-nano-9b-v2", "nvidia/mistral-nemotron"]
    models = _blockrun_models()
    candidates = []
    for model in preferred:
        meta = models.get(model)
        if meta and str(meta.get("billing_mode", "")) == "free" and float(meta.get("pricing", {}).get("input", 1)) == 0 and float(meta.get("pricing", {}).get("output", 1)) == 0:
            candidates.append(model)
    if not candidates:
        raise RuntimeError("BlockRun has no verified free allow-listed model")
    errors = []
    for model in candidates:
        body = {"model": model, "messages": [{"role": "system", "content": "Return exactly one JSON object. No markdown."}, {"role": "user", "content": prompt}], "temperature": 0.1, "max_tokens": 12000, "response_format": {"type": "json_object"}}
        try:
            x = _http_post("https://blockrun.ai/api/v1/chat/completions", body, {"Authorization": "Bearer not-needed-for-free-models", "Content-Type": "application/json"}, retries=2)
            print(f"BLOCKRUN_INFERENCE=PASS model={model}")
            return _extract(((x.get("choices") or [{}])[0].get("message") or {}).get("content", ""))
        except Exception as e:
            msg = str(e)
            errors.append(f"{model}: {msg}")
            print(f"BLOCKRUN_MODEL_SKIP model={model} reason={msg}")
            if "400" in msg.lower() or "response_format" in msg.lower():
                try:
                    body.pop("response_format", None)
                    x = _http_post("https://blockrun.ai/api/v1/chat/completions", body, {"Authorization": "Bearer not-needed-for-free-models", "Content-Type": "application/json"}, retries=2)
                    print(f"BLOCKRUN_INFERENCE=PASS model={model} mode=plain_json")
                    return _extract(((x.get("choices") or [{}])[0].get("message") or {}).get("content", ""))
...
def _cohere(key, prompt):
    model = os.getenv("COHERE_MODEL", "command-r7b-12-2024")
    return _compat("Cohere", key, model, prompt, base_url="https://api.cohere.com/compatibility/v1")
...
def build_long_story_router():
    ...
    if os.getenv("BLOCKRUN_FREE_ENABLED", "true").lower() == "true":
        providers.append(Provider("BlockRun", ["long_story"], 15, True, _blockrun, model="blockrun-free-pool"))
    ...
    if os.getenv("GEMINI_API_KEY"):
        providers.append(Provider("Gemini", ["long_story"], 40, True, lambda p: gemini(os.environ["GEMINI_API_KEY"], p), model=os.getenv("GEMINI_MODEL")))
