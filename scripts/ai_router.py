...
def _classify(exc):
    ...
    return "UNKNOWN"
@dataclass
class Provider: name:str; task_types:list[str]; priority:int; free_only:bool; call:Callable[[str],Any]; model:str|None=None
class AIRouter:
    def __init__(self,providers,task="long_story"):
        ...
        self.providers=sorted([p for p in providers if task in p.task_types or "*" in p.task_types],key=lambda p:p.priority); self.state=_load_state(); self.task=task
