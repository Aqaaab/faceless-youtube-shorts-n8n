...
def story_fixture(duration: float) -> Story:
    families=[
        ('performance','performance: استجابة القوة والتسارع'),
        ('interior','interior: تصميم المقصورة والشاشة'),
        ('technology','technology: منظومة الاستشعار والبرمجيات'),
        ('safety','safety: الفرامل والحماية النشطة'),
        ('charging','charging: منحنى الشحن السريع بجهد 800V'),
        ('efficiency','efficiency: البطارية والمدى واستهلاك الطاقة'),
        ('design','design: خطوط الهيكل والديناميكا الهوائية'),
        ('price','price: القيمة والسعر مقابل المواصفات'),
        ('hero','hero: لقطة افتتاحية للسيارة كاملة'),
        ('performance','performance: القوة والعزم تحت التسارع'),
        ('interior','interior: تجربة المقصورة والواجهة الرقمية'),
        ('technology','technology: الحساسات والكاميرات والمساعدة'),
        ('safety','safety: أنظمة التصادم والتوقف الآمن'),
        ('charging','charging: إدارة الحرارة أثناء الشحن'),
        ('efficiency','efficiency: إدارة الطاقة والمدى الحقيقي'),
        ('design','design: تفاصيل السطح والجنوط والانسيابية'),
        ('price','price: تموضع السعر والقيمة الهندسية'),
        ('hero','hero: لقطة ختامية للسيارة في المشهد'),
        ('performance','performance: استجابة الدفع والقوة القصوى'),
        ('interior','interior: مساحة الركاب وتجربة القيادة'),
        ('technology','technology: بنية النظام والمساعد الذكي'),
        ('safety','safety: مراقبة الطريق والحماية الوقائية'),
        ('charging','charging: سرعة استعادة الطاقة'),
        ('efficiency','efficiency: الكفاءة والاستهلاك تحت الحمل'),
        ('design','design: الإضاءة والخطوط الخارجية'),
    ]
    scenes=[]
...
def build_smoke():
    ...
    story=story_fixture(1.2); generate_visuals(story,WORK/'scenes'); generate_vertical_visuals(story,WORK/'vertical_scenes')
    master_frames=svg_to_pngs(story); vertical_frames=svg_to_pngs(story,True)
    master=WORK/'test_master.mp4'; short=WORK/'test_short_1.mp4'
    make_video([master_frames/f'scene_{s.id:02d}.png' for s in story.scenes],master,'1920:1080',30.0)
    make_video([vertical_frames/f'scene_{s.id:02d}.png' for s in story.scenes],short,'1080:1920',30.0)
...
def build_production():
    if not (WORK/'frames').is_dir(): raise SystemExit('production_render requires successful artifact_gate output')
    story=story_fixture(17.0); master_frames=WORK/'frames'; vertical_frames=WORK/'vertical_frames'
    ...
    full_master=prod/f'{car}_{date}_0.mp4'; make_video([master_frames/f'scene_{s.id:02d}.png' for s in story.scenes],full_master,'1920:1080',425.0)
    failed=[]
    for idx,(a,b) in enumerate(((1,2),(7,8),(13,14),(19,20)),1):
        short=prod/f'{car}_{date}_{idx}.mp4'; frames=[vertical_frames/f'scene_{i:02d}.png' for i in range(a,b+1)]
    ...
    report=json.loads((WORK/'qa_report.json').read_text(encoding='utf-8')); report.update({'failed_shorts':failed,'production_master':str(full_master),'production_shorts':[str(p) for p in sorted(prod.glob(f'{car}_{date}_*.mp4')) if p!=full_master],'cost_usd':0.0,'paid_services_used':[]})
    (WORK/'qa_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
...
def main():
    ...
    if args.production: build_production()
    else: build_smoke()
