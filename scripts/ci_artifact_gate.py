...
def story_fixture(duration: float) -> Story:
    families=[('بطارية','البطارية وتوزيع الخلايا'),('شحن','الشحن السريع بجهد 800V'),('مقصورة','المقصورة والشاشة'),('عجلة','العجلات والفرامل'),('ديناميكية','الديناميكا الهوائية'),('أداء','الأداء والتسارع'),('أمان','أنظمة الأمان'),('تقنية','تقنيات الاستشعار'),('تصميم','تفاصيل التصميم'),('مدى','الكفاءة والمدى'),('سرعة','استجابة الأداء'),('تصميم','خطوط الهيكل'),('تقنية','البرمجيات'),('أمان','الحماية النشطة'),('عجلة','تفاصيل الجنط'),('شحن','منحنى الشحن'),('بطارية','إدارة الطاقة'),('مقصورة','تجربة المقصورة'),('ديناميكية','تدفق الهواء'),('أداء','القوة والعزم'),('تقنية','المساعدات الذكية'),('أمان','التوقف الآمن'),('تصميم','الإضاءة الخارجية'),('مدى','استهلاك الطاقة'),('أداء','الخلاصة والأداء')]
    scenes=[]
...
def build_smoke():
    ...
    report={'car_first_ratio':gate['car_first_ratio'],'gate_pass':bool(gate['passed']),'scenes_total':25,'scenes_car_primary':gate['metrics']['car_first_scenes'],'timestamp':datetime.now(timezone.utc).isoformat(),'source_video':str(master),'gate_score_10':10.0 if gate['passed'] else 0.0}
...
def build_production():
    ...
    report=json.loads((WORK/'qa_report.json').read_text(encoding='utf-8')); report.update({'failed_shorts':failed,'production_master':str(full_master),'production_shorts':[str(p) for p in sorted(prod.glob(f'{car}_{date}_*.mp4')) if p!=full_master]})
    (WORK/'qa_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    if failed: print(json.dumps({'failed_shorts':failed},ensure_ascii=False))
