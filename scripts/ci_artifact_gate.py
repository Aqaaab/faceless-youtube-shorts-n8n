...
def story_fixture(duration:float)->Story:
    ...
    for sid in range(1,26):
        ...
        scenes.append(Scene(sid,narration,desc+f" مع سيارة واضحة وتفصيل هندسي للمشهد {sid}",layouts[(sid-1)%len(layouts)],[desc,f'مشهد {sid}'],duration))
...
def make_video(frames,out,size,duration):
    ...
    run(['ffmpeg','-y','-loglevel','error','-f','concat','-safe','0','-i',str(concat),'-t',str(duration),'-vf',f'scale={size}:flags=lanczos','-c:v','libx264','-preset','veryfast','-pix_fmt','yuv420p','-an',str(out)]); concat.unlink(missing_ok=True)

def write_short_srt(index):
    p=WORK/f'short_segments_{index}'; p.mkdir(parents=True,exist_ok=True)
    text='هذه ترجمة عربية اختبارية واضحة وتبقى ضمن الهوامش الآمنة.\\nمعلومة بصرية مترابطة مع السيارة.'
    (p/'short.srt').write_text(f"1\n00:00:00,000 --> 00:00:17,000\n{text}\n\n2\n00:00:17,000 --> 00:00:34,000\nالمشهد الرأسي يعرض السيارة مع حركة وتفصيل هندسي واضح.\n",encoding='utf-8')

def prepare_frames(duration=1.2):
    if WORK.exists(): shutil.rmtree(WORK)
    WORK.mkdir(parents=True); story=story_fixture(duration); generate_visuals(story,WORK/'scenes'); generate_vertical_visuals(story,WORK/'vertical_scenes'); svg_to_pngs(story); svg_to_pngs(story,True)
...
def build_smoke():
    ...
    report={'gate_version':'v4','gate_pass':bool(gate['passed']),'scenes_total':25,'scenes_car_primary':gate['car_first_scenes'],'visual_product_gate':gate,'timestamp':datetime.now(timezone.utc).isoformat(),'source_video':str(master),'gate_score_10':gate['average_score']/10.0 if gate['passed'] else 0.0,'cost_usd':0.0,'paid_services_used':[]}
...
def build_production():
    story=prepare_frames(17.0); master_frames=WORK/'frames'; vertical_frames=WORK/'vertical_frames'; car='ci_validation_car'; date=datetime.now(timezone.utc).strftime('%Y%m%d'); prod=ROOT/'production_artifacts'
    ...
    for idx,(a,b) in enumerate(((1,2),(7,8),(13,14),(19,20)),1):
        short=prod/f'{car}_{date}_{idx}.mp4'; frames=[vertical_frames/f'scene_{i:02d}.png' for i in range(a,b+1)]; make_video(frames,short,'1080:1920',34.0); write_short_srt(idx)
    ...
    report=json.loads((WORK/'qa_report.json').read_text(encoding='utf-8')) if (WORK/'qa_report.json').is_file() else {}
    report.update({'gate_pass':not failed,'failed_shorts':failed,'production_master':str(full_master),'production_shorts':[str(p) for p in sorted(prod.glob(f'{car}_{date}_*.mp4')) if p!=full_master],'cost_usd':0.0,'paid_services_used':[]}); (WORK/'qa_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
