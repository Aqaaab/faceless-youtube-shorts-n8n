...
def main()->None:
    ...
    plan=json.loads(PLAN.read_text(encoding='utf-8')); story=json.loads(STORY.read_text(encoding='utf-8')); shorts=plan.get('shorts',[]); scenes=story.get('scenes',[])
    if len(shorts)!=4: raise SystemExit(f'EXPECTED_4_SHORTS_GOT_{len(shorts)}')
    if not scenes: raise SystemExit('STORY_SCENES_MISSING')
    total_duration=duration(SOURCE); total_words=max(1,sum(words(s.get('text_en','')) for s in scenes)); cumulative=[0]
