...
BLOCKRUN_MODEL=os.environ.get('BLOCKRUN_VISION_MODEL','nvidia/nemotron-3-nano-omni-30b-a3b-reasoning')
HF_MODEL=os.environ.get('HF_VISION_MODEL','nvidia/nemotron-nano-12b-v2-vl')
...
def _json(t):
 ...
 try:return json.loads(t[a:b+1])
  ...
  except Exception as e:raise RuntimeError('invalid vision JSON') from e
...
def _post(url,body,headers):
 ...
 try:
  ...
  raise RuntimeError(f'HTTP {e.code}: '+e.read().decode('utf-8','replace')[:600]) from e
def _extract(x):
 if isinstance(x,str):return x
 if isinstance(x,dict):
  if isinstance(x.get('response'),str):return x['response']
  c=x.get('choices') or []
