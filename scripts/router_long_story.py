...
def normalize_scene(s):
    s=dict(s); vs=str(s.get('visual_subject','')).strip().split(); q=str(s.get('pexels_query','')).strip().split()
...
def validate_chunk(d,start_scene):
    scenes=d.get('scenes')
    if not isinstance(scenes,list) or len(scenes)!=CHUNK_SIZE: raise ValueError(f'chunk scene count invalid: expected {CHUNK_SIZE}, received {len(scenes) if isinstance(scenes,list) else "non-list"}')
    out=[]
...
def main():
    ...
    for p in providers:
        if not router._eligible(p): print(f'LONG_STORY_PROVIDER_SKIP provider={p.name} reason=cooldown'); continue
        ...
        for chunk_index in range(4):
            ...
            if chunk_index==0: prompt+='Also include topic, category, title (<=90 chars), description (3-5 factual sentences), and 8-15 lowercase ASCII tags.\n'
            else: prompt+='Do NOT include or regenerate title, description, tags, topic, or category; only return the scenes array.\n'
            last=''
            for attempt in range(1,RETRIES+1):
                try:
                    ...
                    scenes=validate_chunk(result,start); all_scenes.extend(scenes)
                    if chunk_index==0: meta={k:result.get(k) for k in ('topic','category','title','description','tags')}
                    print(f'LONG_STORY_CHUNK=PASS provider={p.name} scenes={start}-{end} try={attempt}'); break
        ...
        if failed: continue
        try:
            final=validate_final({**meta,'scenes':all_scenes}); final.update({'provider':p.name,'model':p.model,'router':'Aqaaab AI Router','router_task':'long_story','generation_attempt':1})
            (RUN_DIR/'job.json').write_text(json.dumps(final,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
            print(f'LONG_STORY_ROUTER=PASS provider={p.name} scenes={SCENES} words={final["script_words"]}'); return 0
        except Exception as e: print(f'LONG_STORY_ROUTER provider={p.name} final_validation failed: {e}')
