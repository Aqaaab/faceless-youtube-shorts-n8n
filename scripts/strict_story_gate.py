...
RETRIES = max(1, int(os.getenv("STRICT_STORY_RETRIES", "2")))
...
def _review(story: dict) -> dict:
    payload = {
        ...
        "rules": [
            ...
            "Preserve 25 scenes and the existing chronological story unless a scene is internally contradictory.",
    ...
    for attempt in range(RETRIES):
        try:
            result = extract_json(call(message, model=os.getenv("ODYSSEUS_STORY_MODEL", "aqaaab/story"), timeout=240))
            ...
            if attempt + 1 < RETRIES:
                continue
    raise RuntimeError("STRICT_STORY_GATE failed to obtain a semantic audit/repair") from last
