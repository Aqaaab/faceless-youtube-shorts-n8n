...
BASE_GENERATOR=Path(__file__).with_name('generate_job.py'); MIN_SCENES=int(os.environ.get('MIN_SCENES','5')); MAX_SCENES=int(os.environ.get('MAX_SCENES','10'))
GENERIC_QUERY={'nature','countryside','landscape','background','abstract','object','thing','scene','person','people','random'}; GENERIC_META={'english','general','shorts','unknown','miscellaneous',''}
def tokens(s):return re.findall(r'[a-z0-9]+(?:-[a-z0-9]+)?',s.lower())
def normalize(d):
 sc=d.get('scenes') or []
 if not MIN_SCENES<=len(sc)<=MAX_SCENES:raise SystemExit(f'job.json must contain {MIN_SCENES}-{MAX_SCENES} scenes')
 ...
 for i,s in enumerate(sc,1):
  ...
  q=[x for x in tokens(s.get('pexels_query','')) if x not in GENERIC_QUERY]
  if not q:q=tokens(vs)[:3] or core or tokens(en)[:1]
  if core and core[0] not in q:q=[core[0]]+q
  s['pexels_query']=' '.join(q[:5])
  if 'chameleon' in en.lower() and ('حرباء' not in ar or 'القمل' in ar):raise SystemExit(f'unsafe Arabic translation in scene {i}')
 d['script']=' '.join(s['text_en'].strip() for s in sc);d['narration']=d['script'];d['subtitle_ar']=' '.join(s['text_ar'].strip() for s in sc);d['hook']=sc[0]['text_en'].strip();d['scene_count']=len(sc);d['pexels_query']=sc[0]['pexels_query'];d['provider']=d.get('provider','baseline-fallback')
