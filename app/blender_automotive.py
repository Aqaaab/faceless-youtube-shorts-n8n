...
def render_scene_blender(scene, topic: str, out: Path, size: tuple[int, int], camera: str) -> dict:
    ...
    if meta.get("resolution") != [width, height]:
        raise RuntimeError(f"Blender resolution contract failed: {meta}")
    return {"renderer": meta["renderer"], "camera": camera, "scene_id": getattr(scene, "id", 0), "resolution": [width, height], "output": str(out), "metadata": str(metadata_path), "stdout_tail": proc.stdout[-1200:]}
