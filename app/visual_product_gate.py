...
def _roi_metrics(path:Path,vertical:bool=False)->dict:
    with Image.open(path).convert("L") as im:
        w,h=im.size
        roi=im.crop((0,int(h*.62),w,int(h*.94))) if vertical else im.crop((0,int(h*.72),w,int(h*.96)))
...
def _subtitle_coverage(root:Path,video:Path,vertical:bool)->tuple[bool,str]:
    # Sample a frame near the end; opaque subtitle boxes create a large low-variance dark rectangle.
    ...
    # Do not reject dark automotive backgrounds; only reject the characteristic huge opaque box.
...
def run_visual_product_gate(story:Story,master:Path,shorts:list[Path],report:Path=RUN/"visual_product_gate_v3.json")->dict:
    errors=[]; scenes=[]; families=[]; cameras=[]; intents=[]; hashes=[]
    paths=[]
    for scene in story.scenes:
        ...
        if not svg_path.is_file() or not png_path.is_file():
            errors.append(f"scene {scene.id}: missing rendered visual evidence"); continue
        text=_svg(svg_path)
        ...
        m=_metric(png_path); hashes.append(hash(m["image"].tobytes()))
    ...
    for i in range(len(paths)):
        for j in range(i+1,len(paths)):
            a=_metric(paths[i])["image"]; b=_metric(paths[j])["image"]
            pair_distances.append(_distance(a,b))
    if pair_distances:
        near=sum(1 for d in pair_distances if d<0.055)
        p95=sorted(pair_distances)[max(0,int(len(pair_distances)*.95)-1)]
