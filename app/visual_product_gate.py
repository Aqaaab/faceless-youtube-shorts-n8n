...
def run_visual_product_gate(story:Story,master:Path,shorts:list[Path],report:Path=RUN/"visual_product_gate_v4.json"):
    ...
    for scene in story.scenes:
        ...
        families.append(family); cameras.append(camera); intents.append(intent.casefold()); paths.append(png_path); signatures.append(_car_signature(svg)); row=_scene_score(svg,png_path); row.update({"scene_id":scene.id,"family":family,"camera":camera}); scene_rows.append(row)
    if len(scene_rows)!=25: errors.append(f"visual evidence incomplete: {len(scene_rows)}/25")
    car_ratio=sum(1 for s in signatures if s)/max(1,len(signatures))
    ...
    if len(set(s for s in signatures if s))>1: errors.append("car identity signature drift detected across scenes")
