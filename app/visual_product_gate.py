...
def _probe_subtitle(root:Path,srt:Path,size:tuple[int,int],style:str,name:str)->dict:
    temp=root/name; temp.mkdir(parents=True,exist_ok=True); png=temp/"subtitle_probe.png"; filt=f"color=c=black:s={size[0]}x{size[1]}:d=2,subtitles={srt}:force_style='{style}'"; p=_run(["ffmpeg","-y","-f","lavfi","-i",filt,"-frames:v","1","-vf","format=gray",str(png)],False)
...
def _scene_score(svg,png):
    with Image.open(png).convert("L") as im:
        gsmall=im.resize((96,54)); gs=ImageStat.Stat(gsmall); edge=ImageStat.Stat(gsmall.filter(ImageFilter.FIND_EDGES)).mean[0]; gradient=len(re.findall(r"gradient",svg,re.IGNORECASE)); layers=len(re.findall(r"<(?:path|circle|rect|ellipse|g)\b",svg,re.IGNORECASE)); reflections=len(re.findall(r"opacity=[\x22\x27]0\.(?:0[1-9]|[12][0-9])",svg,re.IGNORECASE)); detail_strokes=len(re.findall(r"<path\b[^>]*\bstroke=",svg,re.IGNORECASE)); anchors=len(re.findall(r"<(?:text|circle|rect)\b",svg,re.IGNORECASE)); car=int('data-car-layer="primary"' in svg); composition=min(100,25+min(25,gradient*4)+min(20,layers/12)+min(15,reflections*2)+min(12,detail_strokes/8)+min(15,anchors/8)+min(10,edge*0.75)); realism=min(100,35+min(25,gradient*4)+min(20,reflections*3)+min(20,detail_strokes/10)+min(10,edge*0.5)); subject=min(100,50+20*car+min(30,edge*2)); return {"composition_score":round(composition,1),"visual_realism_score":round(realism,1),"subject_visibility_score":round(subject,1),"mean_luma":round(gs.mean[0],1),"edge_mean":round(edge,1),"layer_count":layers,"gradient_count":gradient,"reflection_count":reflections,"detail_stroke_count":detail_strokes,"anchor_count":anchors}
