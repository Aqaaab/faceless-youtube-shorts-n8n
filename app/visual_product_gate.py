...
def car_first_ratio(scene_svgs:list[str])->float:
    return sum(1 for text in scene_svgs if re.search(r'data-car-layer=["\']primary["\']',text))/len(scene_svgs) if scene_svgs else 0.0
...
def _roi_metrics(path:Path,vertical:bool=False)->dict:
    with Image.open(path).convert("L") as im:
        w,h=im.size; roi=im.crop((0,int(h*.62),w,int(h*.94))) if vertical else im.crop((0,int(h*.72),w,int(h*.96))); bg=im.crop((0,0,w,max(1,int(h*.10))))
...
def _subtitle_coverage(root:Path,video:Path,vertical:bool)->tuple[bool,str]:
    ...
    p=subprocess.run(["ffmpeg","-y","-ss","1","-i",str(video),"-frames:v","1","-vf","format=gray",str(out)],capture_output=True,text=True)
...
def run_visual_product_gate(story:Story,master:Path,shorts:list[Path],report:Path=RUN/"visual_product_gate_v3.json")->dict:
    ...
    for scene in story.scenes:
        ...
        m=_metric(png_path); scenes.append({"id":scene.id,"family":family,"camera":camera,"mean":round(m["mean"],2),"std":round(m["std"],2)})
    ...
    for i in range(len(paths)):
        for j in range(i+1,len(paths)): pair_distances.append(_distance(_metric(paths[i])["image"],_metric(paths[j])["image"]))
    if pair_distances:
