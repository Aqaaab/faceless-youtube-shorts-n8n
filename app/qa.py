...
def _srt(path: Path, expected_cues: int) -> tuple[bool, str]:
    ...
    if any("MODE_FACT_SOURCE_REQUIRED" in line or "hud_only" in line or "generic" in line for line in text.splitlines()): return False, "subtitle contains internal/debug text"
    return True, "ok"
...
def _visual_product_gate(story: Story) -> tuple[list[str], dict]:
    errors = []; hashes, modes = [], []; quality_scores = []; car_assets = motion_assets = 0
    for s in story.scenes:
        ...
        mode_match = re.search(r'data-visual-mode="([^"]+)"', text); layout_match = re.search(r'data-layout="([^"]+)"', text)
    ...
    unique_assets = len(set(hashes)); unique_modes = len(set(modes)); score = round(sum(quality_scores) / max(1, len(quality_scores)), 1)
    ...
    return errors, {"average_score": score, "car_first_scenes": car_assets, "motion_scenes": motion_assets, "unique_assets": unique_assets, "unique_modes": unique_modes}
...
def qa(story: Story, master: Path, shorts: list[Path], report: Path = RUN/"qa_report.json"):
    ...
    # Weighted production score: this is a gate score, not a substitute for human visual review.
    category = {
        "Script / Story": 0.15, "Visual Quality": 0.25, "Scene Relevance": 0.15,
        "Audio / Voice": 0.10, "Arabic Subtitles": 0.10, "Synchronization": 0.10,
        "Shorts": 0.10, "Metadata / Publishing": 0.05,
    }
    script_score = 10 if not any("scene " in e and "narration" in e for e in errors) else 0
    visual_score = min(10.0, visual_metrics["average_score"] / 10.0)
    audio_score = 10 if not any("audio failed" in e for e in errors) else 0
    subtitle_score = 10 if sub_ok and short_ok else 0
    sync_score = 10 if not any("drift" in e or "TIMING" in e for e in errors) else 0
    shorts_score = 10 if len(shorts) == 4 and not any("short " in e.lower() for e in errors) else 0
    metadata_score = 10 if not any(x in " ".join(errors) for x in ["title", "description", "tags"]) and len(short_titles) == 4 else 0
    scores = {"Script / Story": script_score, "Visual Quality": visual_score, "Scene Relevance": 10 if visual_metrics["unique_modes"] >= 4 else 0, "Audio / Voice": audio_score, "Arabic Subtitles": subtitle_score, "Synchronization": sync_score, "Shorts": shorts_score, "Metadata / Publishing": metadata_score}
