...
def _text(text,x,y,size,weight=500,anchor="start",fill=TEXT):
    value=html.escape(str(text)[:110]);return f'<text x="{x}" y="{y}" font-family="Noto Sans Arabic,Noto Sans,DejaVu Sans,sans-serif" font-size="{size}px" font-weight="{weight}" text-anchor="{anchor}" fill="{fill}">{value}</text>'


def _hero_car(kind:str,scene_id:int):
    variants=[("vertical_hero",-80,470,.72,1),("vertical_low",-120,510,.76,1),("vertical_tight",-185,410,.82,1),("vertical_reverse",1160,470,.72,-1),("vertical_wide",-25,555,.66,1)]
...
def vertical_scene_svg(scene,topic:str,out:Path):
    out.parent.mkdir(parents=True,exist_ok=True);kind=_kind(scene);layout=scene.layout.casefold();calls=[str(c) for c in scene.callouts[:3]];intent=str(scene.visual_intent).strip();angle,car=_hero_car(kind,scene.id)
    svg=f'''<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" data-visual-mode="{html.escape(kind)}" data-layout="{html.escape(layout)}" data-camera-angle="{angle}" data-asset-quality="premium_automotive_editorial_v2_vertical" data-motion="vertical_push_pan">
      {_defs()}<rect width="1080" height="1920" fill="#07090c"/><rect width="1080" height="1500" fill="url(#bg)"/><ellipse cx="540" cy="840" rx="510" ry="430" fill="url(#spot)"/>
      <path d="M55 120 H1025" stroke="{ACCENT}" stroke-width="4"/>{_text(topic,55,85,28,700,"start",TEXT)}{car}{_focus_overlay(kind,scene)}{_callout_stack(calls)}
      <rect x="60" y="1720" width="960" height="135" rx="24" fill="#080B0F" stroke="#303944"/>{_text(intent,92,1775,23,550,"start",TEXT)}{_text(MODE_LABELS.get(kind,"AUTOMOTIVE"),540,1835,18,700,"middle",MUTED)}
    </svg>'''
