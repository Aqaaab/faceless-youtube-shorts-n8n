...
def _focus_art(family: str, scene, y: int = 1030) -> str:
    ...
    if family == "wheel_detail":
        return f'<g transform="translate(540 {y+120})"><circle cx="0" cy="0" r="190" fill="#050709" stroke="#DDE4E9" stroke-width="11"/><circle cx="0" cy="0" r="125" fill="#202A34" stroke="{ACCENT}" stroke-width="6"/><path d="M-82 0 H82 M0 -82 V82" stroke="#BFC8CF" stroke-width="5"/><text x="-180" y="270" font-family="Noto Sans" font-size="22" fill="{MUTED}">DETAIL / CONTROL</text></g>'
    if family in {"technology","battery","charging","safety"}:
        label={"technology":"TECHNOLOGY","battery":"ENERGY","charging":"CHARGING","safety":"SAFETY"}[family]; dots=''.join(f'<circle cx="{180+i*230}" cy="{y+115}" r="18" fill="{ACCENT}"/>' for i in range(4))
...
def vertical_scene_svg(scene, topic: str, out: Path) -> None:
    ...
    if kind not in SEMANTIC_MODES: kind="design" if family in {"design_detail","aero"} else "performance" if family=="performance" else "technology" if family in {"technology","battery","charging"} else "safety" if family=="safety" else "interior" if family=="interior" else "price" if family=="comparison" else "efficiency"
    motion=("push_in","pull_out","orbit_left","orbit_right","rack_focus","tracking","rise")[(int(scene.id)+len(family))%7]
    ...
    elif family in {"front_3q","rear_3q","side_profile"}: car=_car("translate(-170 300) scale(.80)")
    ...
    svg=f'''<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" data-visual-family="{family}" data-visual-mode="{kind}" data-layout="{html.escape(str(scene.layout))}" data-camera-angle="vertical_{family}" data-visual-intent="{html.escape(intent[:240])}" data-motion="{motion}" data-car-layer="primary" data-asset-quality="premium_automotive_vertical_v4">{_defs()}<rect width="1080" height="1920" fill="#07090C"/><rect width="1080" height="1500" fill="url(#bg)"/><ellipse cx="540" cy="760" rx="510" ry="520" fill="url(#halo)"/><path d="M50 115 H1030" stroke="{ACCENT}" stroke-width="4"/>{_text(topic,topic_x,82,30,700,"start",TEXT)}{car}<path d="M70 1030 H1010" stroke="#25303A" stroke-width="3"/>{overlay}{cards}{_text(kind.upper(),540,1780,21,700,"middle",MUTED)}</svg>'''
