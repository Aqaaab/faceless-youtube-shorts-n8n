...
def service():
    credentials = Credentials(None, refresh_token=os.environ["YOUTUBE_REFRESH_TOKEN"], token_uri="https://oauth2.googleapis.com/token", client_id=os.environ["YOUTUBE_CLIENT_ID"], client_secret=os.environ["YOUTUBE_CLIENT_SECRET"], scopes=SCOPES)
...
def _final_title(title, marker):
    ...
    if len(base) + len(marker) <= MAX_TITLE_CHARS: return base + marker
...
def existing_titles(svc, marker):
    ...
    if not channels: raise RuntimeError("YouTube OAuth succeeded but no channel is accessible to this token")
    ...
    while True:
        ...
        out.extend(item.get("snippet", {}).get("title", "") for item in data.get("items", []) if item.get("snippet"))
...
def _require_final_qa(root: Path) -> dict:
    ...
    if not report.exists() or not master.is_file() or any(not p.is_file() for p in shorts): raise RuntimeError("UPLOAD BLOCKED: final production artifact/QA report is incomplete")
    ...
    if data.get("passed") is not True: raise RuntimeError("UPLOAD BLOCKED: final QA report is not passed")
    visual = data.get("visual_product_gate", {})
    if not visual or float(visual.get("average_score", 0)) < 85: raise RuntimeError("UPLOAD BLOCKED: visual product gate evidence is missing or below threshold")
    return data
...
def main():
    ...
    if not isinstance(short_titles, list) or len(short_titles) != 4: raise RuntimeError("UPLOAD BLOCKED: four validated Short hooks are required")
    items = [("master_final.mp4", story["title"], "long")]
    items += [(f"shorts/short_{i}.mp4", short_titles[i-1], f"short_{i}") for i in range(1, 5)]
    for rel, title, kind in items:
        path = root / rel
        if not path.is_file() or not path.stat().st_size: raise FileNotFoundError(f"Missing upload artifact: {path}")
        fp = fingerprint(path)
