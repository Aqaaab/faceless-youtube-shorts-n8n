...
def validate_story(path=Path('work/story.json')):
    ...
    if not MIN_LONG<=total<=MAX_LONG: errors.append(f'planned duration {total:.1f}s outside 420-900')
    layouts=[]; intents=[]; callout_scenes=0
    for s in scenes:
        sid=s.get('id')
        for k in ('id','narration','visual_intent','layout','duration'):
            if s.get(k) in (None,'',[]): errors.append(f'scene {sid} missing {k}')
        try: d=float(s.get('duration',0))
        except (TypeError,ValueError): d=0; errors.append(f'scene {sid} duration is not numeric')
        if not MIN_SCENE<=d<=MAX_SCENE: errors.append(f'scene {sid} duration {d:.1f}s outside 5-60')
        narration=str(s.get('narration','')).strip(); intent=str(s.get('visual_intent','')).strip(); layout=str(s.get('layout','')).strip().lower()
        if _words(narration)<25: errors.append(f'scene {sid} narration too short ({_words(narration)} words; minimum 25)')
