...
def _validate_callouts(callouts, narration: str, sid: int) -> list[str]:
    errors=[]; narration_text=str(narration); narration_lower=narration_text.casefold(); narration_numbers=set(_numeric_tokens(narration_text)); narration_words=_lexical_tokens(narration_text)
    for callout in callouts:
        if not isinstance(callout,str) or not callout.strip(): errors.append(f"scene {sid} callouts must contain non-empty strings"); continue
        value=callout.strip()
        for token in _numeric_tokens(value):
            if token not in narration_numbers: errors.append(f"scene {sid} callout introduces unsupported numeric token: {token}")
        callout_words=_lexical_tokens(value)
        if callout_words and narration_words and not (callout_words & narration_words): errors.append(f"scene {sid} callout has no lexical support in narration: {value}")
        if value.casefold() not in narration_lower and not _numeric_tokens(value) and callout_words and not (callout_words & narration_words): errors.append(f"scene {sid} callout is not grounded in narration: {value}")
    return errors


def _validate_short_titles(data: dict, errors: list[str]) -> None:
    titles=data.get("short_titles")
    if not isinstance(titles,list) or len(titles)!=4: errors.append("exactly 4 short_titles are required"); return
    normalized=[]
    for index,title in enumerate(titles,1):
        value=str(title).strip(); folded=value.casefold(); normalized.append(folded)
        if not 20<=len(value)<=80: errors.append(f"short title {index} must be 20-80 characters")
        if not _has_arabic(value): errors.append(f"short title {index} must contain Arabic text")
        if folded in {"short 1","short 2","short 3","short 4","untitled","untitled story"}: errors.append(f"short title {index} is generic")
        if "\ufffd" in value: errors.append(f"short title {index} contains replacement glyph U+FFFD")
    if len(set(normalized))!=4: errors.append("Short titles must be unique")
...
def validate_story_data(data: dict) -> bool:
    ...
    errors=[]
    if "\ufffd" in json.dumps(data,ensure_ascii=False): errors.append("payload contains replacement glyph U+FFFD")
    ...
    for index,scene in enumerate(scenes,1):
        ...
        except (TypeError,ValueError): duration=0.0; errors.append(f"scene {sid} duration is not numeric")
        durations.append(duration)
        if not MIN_SCENE<=duration<=MAX_SCENE: errors.append(f"scene {sid} duration {duration:.2f}s outside {MIN_SCENE:g}-{MAX_SCENE:g}s")
        narration=str(scene.get("narration","")).strip(); intent=str(scene.get("visual_intent","")).strip(); layout=str(scene.get("layout","")).strip().lower(); aggregate.append(narration)
        words=_words(narration)
        if words<MIN_WORDS or words>MAX_WORDS: errors.append(f"scene {sid} narration must be {MIN_WORDS}-{MAX_WORDS} words (got {words})")
    ...
    if callout_scenes<12: errors.append(f"callout coverage too low: {callout_scenes}/25")
    ...
    for number,group in enumerate(SHORT_GROUPS,1):
        if all(scene_id in by_id for scene_id in group):
            try: pair_duration=sum(float(by_id[scene_id]["duration"]) for scene_id in group)
            except (TypeError,ValueError,KeyError): errors.append(f"Short {number} source duration is invalid")
            else:
                if not SHORT_MIN<=pair_duration<=SHORT_MAX: errors.append(f"Short {number} source scenes {group} total {pair_duration:.2f}s outside {SHORT_MIN:g}-{SHORT_MAX:g}s")
    title=str(data.get("title","")).strip(); description=str(data.get("description","")).strip(); tags=data.get("tags")
    if not title or title.casefold() in {"untitled","untitled story"}: errors.append("weak or missing title")
    elif not 20<=len(title)<=100: errors.append("title must be 20-100 characters")
    ...
    aggregate_text=str(data.get("narration","")).strip() or " ".join(aggregate).strip()
