...
def render_long(story: Story, out: Path=RUN/'master.mp4'):
    ...
    for s in story.scenes:
        ...
        _run(['ffmpeg','-y','-loop','1','-i',str(frame),'-i',str(audio),'-t',str(s.duration),'-vf','scale=1920:1080','-af','apad','-c:v','libx264','-pix_fmt','yuv420p','-c:a','aac',str(seg)])
...
def burn_subtitles(src,srt,out):
    ...
    marker={'burned':True,'source':src.name,'output':out.name,'subtitle_file':str(srt),'subtitle_sha256':hashlib.sha256(Path(srt).read_bytes()).hexdigest()}
...
def render_shorts(story: Story,out_dir: Path=RUN/'shorts'):
    ...
    starts=[0,6,12,18]; evidence=[]
    for idx,start in enumerate(starts,1):
        selected=story.scenes[start:start+2]; segs=RUN/f'short_segments_{idx}'; segs.mkdir(exist_ok=True,parents=True); files=[]
        for s in selected:
            ...
            _run(['ffmpeg','-y','-loop','1','-i',str(frame),'-i',str(audio),'-t',str(s.duration),'-vf','scale=1080:1920','-af','apad','-c:v','libx264','-pix_fmt','yuv420p','-c:a','aac',str(seg)])
            files.append(seg)
        ...
        srt.write_text('\n'.join(rows),encoding='utf-8')
        out=out_dir/f'short_{idx}.mp4'; extra=max(0,45-t); vf=f"subtitles={srt}:force_style='FontName=DejaVu Sans,FontSize=22,Alignment=2,MarginV=85',tpad=stop_mode=clone:stop_duration={extra},scale=1080:1920"
        _run(['ffmpeg','-y','-i',str(raw),'-t','45','-vf',vf,'-af','apad','-t','45','-c:v','libx264','-crf','20','-pix_fmt','yuv420p','-c:a','aac',str(out)])
        evidence.append({'file':str(out),'burned':True,'srt':str(srt),'subtitle_sha256':hashlib.sha256(srt.read_bytes()).hexdigest()})
