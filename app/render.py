...
def render_long(story: Story, out: Path = RUN / "master.mp4"):
    ...
    for s in story.scenes:
        frame = frames / f"scene_{s.id:02d}.png"
...
def burn_subtitles(src: Path, srt: Path, out: Path):
    _run([
        ...
        f"subtitles={srt}:force_style='FontName=Noto Sans Arabic,FontSize=24,Alignment=2,MarginV=55,Outline=2,Shadow=1'",
        "-c:v", "libx264", "-crf", "20", "-pix_fmt", "yuv420p", "-c:a", "copy", str(out)
    ])
    marker = {
        "burned": True,
        "source": src.name,
...
def render_shorts(story: Story, out_dir: Path = RUN / "shorts"):
    ...
    for idx, scene_ids in enumerate(groups, 1):
        ...
        files = []
        for s in selected:
            frame = segs / f"s{s.id}.png"
            seg = segs / f"s{s.id}.mp4"
            audio = RUN / "audio" / f"scene_{s.id:02d}.mp3"
            _render_image(RUN / "vertical_scenes" / f"scene_{s.id:02d}.svg", frame, "1080:1920")
            _render_segment(frame, audio, float(s.duration), seg, "1080:1920")
            files.append(seg)
        cat = segs / "cat.txt"
        ...
        target = min(45.0, max(28.0, t))
        extra = max(0.0, target - t)
        vf = (
            f"subtitles={srt}:force_style='FontName=Noto Sans Arabic,FontSize=22,Alignment=2,"
            f"MarginV=85,Outline=2,Shadow=1',"
            f"tpad=stop_mode=clone:stop_duration={extra},scale=1080:1920"
        )
        _run([
            "ffmpeg", "-y", "-i", str(raw), "-t", str(target), "-vf", vf,
            "-af", f"apad=pad_dur={target},atrim=duration={target}",
            "-c:v", "libx264", "-crf", "20", "-pix_fmt", "yuv420p", "-c:a", "aac", str(out)
        ])
        evidence.append({
            "file": str(out),
            "burned": True,
