...
def render_shorts(story: Story, out_dir: Path = RUN / "shorts"):
    ...
    for idx, scene_ids in enumerate(groups, 1):
        ...
        srt.write_text("\n".join(rows), encoding="utf-8")
        out = out_dir / f"short_{idx}.mp4"; style = "FontName=Noto Sans Arabic,FontSize=21,Alignment=2,MarginV=92,Outline=2,Shadow=0,BorderStyle=1,Spacing=0,WrapStyle=2"; vf = f"subtitles={srt}:force_style='{style}',scale=1080:1920:force_original_aspect_ratio=increase:flags=lanczos,crop=1080:1920:(in_w-1080)/2:(in_h-1920)/2,setsar=1"
