...
from .visuals import generate_vertical_visuals
...
def render_shorts(story: Story,out_dir: Path=RUN/"shorts"):
    ...
    for idx,start in enumerate(starts,1):
        ...
        segs=RUN/f"short_segments_{idx}"; segs.mkdir(exist_ok=True,parents=True)
        files=[]
        for s in selected:
        ...
        raw=segs/"raw.mp4"; _run(["ffmpeg","-y","-f","concat","-safe","0","-i",str(cat),"-c","copy",str(raw)])
        # Shorts are native vertical compositions, then subtitles are burned into the vertical output.
        srt=segs/"short.srt"; t=0; rows=[]
        def ts(x):
            ms=int(round((x-int(x))*1000)); sec=int(x); return f"{sec//3600:02d}:{(sec%3600)//60:02d}:{sec%60:02d},{ms:03d}"
        for n,s in enumerate(selected,1): rows.append(f"{n}\n{ts(t)} --> {ts(t+s.duration)}\n{s.narration}\n"); t+=s.duration
        srt.write_text('\n'.join(rows),encoding='utf-8')
        out=out_dir/f"short_{idx}.mp4"; dur=min(59,max(28,t))
        _run(["ffmpeg","-y","-i",str(raw),"-t",str(dur),"-vf",f"subtitles={srt}:force_style='FontName=DejaVu Sans,FontSize=22,Alignment=2,MarginV=85'","-c:v","libx264","-crf","20","-c:a","copy",str(out)])
