...
W,H=1920,1080
BG="#07090c";TEXT="#F4F6F8";MUTED="#A7AFB8";ACCENT="#E8B44A";PANEL="#11161c";LINE="#303944"
...
def _kind(scene):
    text=(scene.narration+" "+scene.visual_intent).casefold();groups={
...
def render_scene_svg(scene,topic:str,out:Path)->None:
    ...
    topic_x=1810 if _has_arabic(safe_topic) else 70;topic_anchor="start"
    intent_x=1810 if _has_arabic(intent) else 105
    svg=f'''<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" data-visual-mode="{html.escape(kind)}" data-layout="{html.escape(layout)}" data-camera-angle="{camera}" data-asset-quality="premium_automotive_editorial_v2" data-motion="camera_push_pan">{_defs()}{_environment()}<path d="M70 105 H1850" stroke="{ACCENT}" stroke-width="3" opacity=".65"/>{_text(safe_topic,topic_x,78,29,700,topic_anchor,TEXT)}{car_transform}{_semantic_overlay(kind,scene)}{_chips(calls)}<rect x="70" y="915" width="1780" height="95" rx="22" fill="#080B0F" fill-opacity=".88" stroke="#303944"/>{_text(intent,intent_x,973,24,550,"start",TEXT)}</svg>'''
