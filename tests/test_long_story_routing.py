...
def make_scene(i):
    ...
    return {
        "text_en": en,
        "text_ar": "يكشف هذا المشهد تفصيلاً واضحاً في التحقيق، ويوضح أهمية الأدلة وكيف غيّرت القرائن مسار القضية.",
        "visual_subject": "archival evidence",
        "pexels_query": "archival evidence investigation",
        "beat": ("hook", "setup", "mystery", "escalation", "evidence", "reveal", "payoff", "ending")[i % 8],
    }
...
def test_validate_contract():
    ...
    doc={
        "topic":"Regression test story",
        "category":"Stories",
        "title":"Regression Test Story",
        "description":"This is a test story sentence. It validates the long-form contract. It contains no production claims.",
        "tags":["story","mystery","history","evidence","research","discovery","documentary","explained"],
        "scenes":scenes,
    }
    out=validate(doc)
    ...
    bad=dict(doc)
    bad["scenes"]=scenes[:17]
    try:
        validate(bad)
    except ValueError as exc:
        assert "scene count" in str(exc)
    else:
        raise AssertionError("17 scenes must be rejected")
...
def test_router_fallback_after_rate_limit():
    ...
    def succeed(_):
        ...
        return {"ok": True}
    router=AIRouter([
        Provider("first",["long_story"],1,True,fail,"m1"),
        Provider("second",["long_story"],2,True,succeed,"m2"),
    ])
...
if __name__=="__main__":
    ...
    test_router_fallback_after_rate_limit()
    print("LONG_STORY_ROUTING_REGRESSION=PASS")
