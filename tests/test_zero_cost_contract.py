...
def test_contract_zero_cost_05_invalid_json_retries(monkeypatch):
    ...
    class Response:
        status_code=200;headers={};ok=True
        def __init__(self,content):self._content=content;self.text=json.dumps(content)
        def json(self):return {"response":self._content}
    responses=iter([Response("not-json"),Response(json.dumps({"ok":True}))]);calls=[]
    def post(*args,**kwargs):calls.append(kwargs);return next(responses)
    monkeypatch.setenv("ODYSSEUS_GATEWAY_BASE_URL","https://example.invalid");monkeypatch.setenv("ODYSSEUS_GATEWAY_API_KEY","test");monkeypatch.setenv("ODYSSEUS_MAX_ATTEMPTS","3")
    monkeypatch.setattr(core.requests,"post",post);monkeypatch.setattr(core.time,"sleep",lambda _:None)
    assert core.ask_odysseus("system","user")=={"ok":True};assert len(calls)==2
